...
export async function googleAuth(idToken: string) {
    ...
    if (existing) {
        ...
        user = { ...existing, googleId: payload.sub, avatar: payload.picture || existing.avatar } as User;
    } else {
        const newUser = {
            email: payload.email,
...
export async function getDemoCredentials() {
    ...
    if (!user) {
        ...
        user = { _id: result.insertedId, email: demoEmail, passwordHash, name: 'Demo User', createdAt: now, updatedAt: now } as User;
    }
